...
function Calendar(id, size, labelSettings, colors, options) {
    ...
    if (options.lang == undefined) options.lang='it';

    //this.alldaylabel = 'Tutto il giorno';
    this.alldaylabel = GlobalTranslationArray.tuttoilgiorno;
    if (options.alldaylabel != undefined) this.alldaylabel = options.alldaylabel;
    ...
    listPlaceholder.className = "planning-list-placeholder";
    //listPlaceholder.appendChild(document.createTextNode("Non ci sono eventi disponibili"));
    listPlaceholder.appendChild(document.createTextNode(GlobalTranslationArray.noeventi));
    ...
    var months = GlobalTranslationArray.arraymesi;
    if (options.months != undefined && options.months.length == 12) months = options.months;

    //var label = ["Domenica", "Lunedì", "Martedì", "Mercoledì", "Giovedì", "Venerdì", "Sabato"];
    var label = GlobalTranslationArray.arraygiorni;
    ...
    this.labels = [];
    for (var i = 0; i < 7; i++) {
        this.label.push(label[labelSettings[0] + this.label.length >= label.length ? Math.abs(label.length - (labelSettings[0] + this.label.length)) : labelSettings[0] + this.label.length]);
    }
    for (var i = 0; i < 7; i++) {
...
Calendar.prototype.draw = function () {
    ...
    theCalendar.className = "planning-calendar planning-size-" + this.size;
    ...
    for (var i = 0; i < theNames.length; i++) {
        ...
        theContainers[i].className = "planning-" + theNames[i];
        ...
        if (theNames[i] != "days") {
            if (theNames[i] != "month") {
                ...
                if (theNames[i] != "labels") {
                    var backSlider = document.createElement("button");
                    backSlider.id = this.id + "-year-back";
                    backSlider.setAttribute("aria-label", "Anno precedente");
                    ...
                    var nextSlider = document.createElement("button");
                    nextSlider.id = this.id + "-year-next";
                    nextSlider.setAttribute("aria-label", "Anno successivo");
                ...
                backSlider.setAttribute("aria-label", "Mese precedente");
                backSlider.insertAdjacentHTML('beforeend', backSvg);
                theContainers[i].appendChild(backSlider.cloneNode(true));
                ...
                nextSlider.setAttribute("aria-label", "Mese successivo");
                nextSlider.insertAdjacentHTML('beforeend', nextSvg);
                theContainers[i].appendChild(nextSlider.cloneNode(true));
            }
        }
    ...
    for (var i = 0, j = 0; i < 42; i++) {
        ...
        if (this.indicator) {
            ...
            theIndicator.className = "planning-day-indicator planning-indicator-pos-" + this.indicator_pos;
                if (this.indicator_type == 1) theIndicator.className += " planning-indicator-type-numeric";
    ...
    document.getElementById(this.id).innerHTML = "<style>.planning-day-indicator { color: " + this.colors[1] + "; background-color: " + this.colors[1] + "; } .planning-indicator-type-numeric { color: " + this.colors[2] + "; } .planning-day.planning-day-today > .planning-day-num { border-color: " + this.colors[1] + " !important; }</style>";
    document.getElementById(this.id).appendChild(theCalendar.cloneNode(true));
};
...
Calendar.prototype.update = function () {
    ...
    for (var i = 1; i <= 42; i++) {
        ...
        document.getElementById(this.id + '-day-' + i).className = this.id + " planning-day planning-day-listed";
    }
    ...
    for (var i = 0, j = previousLastDay; i < firstDayLabelPos; i++, j--) {
        ...
        document.getElementById(this.id + '-day-' + (firstDayLabelPos - i)).className = this.id + " planning-day planning-day-diluted";
    }
    ...
    for (var i = 1; i <= lastDay; i++) {
        ...
        if (this.date.getMonth() == this.today.getMonth())
            if (i == this.today.getDate()) document.getElementById(this.id + '-day-' + (firstDayLabelPos + i)).className += " planning-day-today";
    }
...
Organizer.prototype.draw = function () {
    ...
    theOrganizer.className = "planning-events planning-size-" + this.calendar.size;

    var theDate = document.createElement("DIV");
    theDate.className = "planning-date";
    ...
    backSlider.setAttribute("aria-label", "Giorno precedente");
    backSlider.insertAdjacentHTML('beforeend', backSvg);
...
Organizer.prototype.list = function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].startTime=='00:00' && data[i].endTime=='24:00')
        {
            span.appendChild(document.createTextNode(this.calendar.alldaylabel));
        }
        else
        {
        ...
        logo.title = data[i].calendar;
        logo.alt = 'Icona di '+data[i].calendar;

        if (data[i].href!='')
        {
            var a = document.createElement('a');
            var linkText = document.createTextNode(data[i].text);
            a.appendChild(linkText);
            a.title = data[i].text;
            
            if (data[i].href.indexOf('schoolplanner://')!=-1)
            {
                var url_tmp=data[i].href.replace('schoolplanner://', '');
                var array_url_tmp=url_tmp.split(':');
                a.classList.add('schoolplanner_event');
                a.href = '#';
                a.setAttribute('data-id', array_url_tmp[0]);
                a.setAttribute('data-tipo', array_url_tmp[1]);
            }
            else
            {
                a.href = data[i].href;
...
Organizer.prototype.remember = function (date, content) {
    if (content.startsWith("<div class=\"planning-list-placeholder\">"))
        return "";
    ...
    var container = document.createElement("UL");
    container.className = "planning-list planning-list-history"

    var title = document.createElement("LI");
    title.appendChild(document.createTextNode(dateTitle));
    title.className = "planning-list-history-title planning-date";
...
Organizer.prototype.showPlaceholder = function (data) {
    return this.calendar.placeholder;
    
    /*var container = document.createElement("DIV");
    container.className = "planning-list-placeholder";
...
Organizer.prototype.indicateEvents = function (data) {
    ...
    if (this.calendar.indicator) {
        var allDays = document.getElementsByClassName(this.calendar.id + " planning-day planning-day-listed");
